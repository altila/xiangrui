...
class Snoopy {
	...
	var $stripRepStr    =   "";                 // strip replace string
		...
		switch( strtolower($URI_PARTS["scheme"]) ) {
			...
			case "https":
				...
				if(function_exists("is_executable"))
					if ( !is_executable($this->curl_path) ) return false;
				$this->host = $URI_PARTS["host"];
				if( !empty($URI_PARTS["port"]) ) $this->port = $URI_PARTS["port"];
				if( $this->_isproxy ) {
					// using proxy, send entire URI
...
\*======================================================================*/
	...
	function _httprequest($url,$fp,$URI,$http_method,$content_type="",$body="") {
		if( $this->lastUrl != $url ) unset($this->results);
		$cookie_headers = '';
		...
		if( !empty($this->host) && !isset($this->rawheaders['Host']) ) {
			...
			if( !empty($this->port) ) $headers .= ":".$this->port;
			$headers .= "\r\n";
		}
